...
**100円noteの「下書き」を1日1本、自動で作り続ける“ネタ工場”。**
外部APIキー不要・課金ゼロ。素材バンク × 日付シードで、毎日ちがう100円noteの下書きを生成します。
テーマは **仕事・働き方・キャリア**（会議・報連相・転職・副業など27題）× 論理の型7種。
...
**「全自動で稼ぐ」は現実には成立しません。** 理由は3つ：

1. **売れない** — テンプレ量産記事は「どこかで読んだ感」が出て、100円を払ってもらいにくい。
2. **アカBANリスク** — note に公式の投稿APIは無く、毎日の自動投稿はBOT判定で凍結の恐れ。
3. **少額** — フォロワーゼロの新規アカでは、売れても1日数十円。

なので本ツールは **「稼ぐための道具」ではなく「毎日ラクに書くための道具」** として設計しています。
稼いでいる人のやり方に寄せた、現実的なワークフローがこちら：

```
① 自動：毎日ネタ＆下書きが1本できる        ← このツール（ネタ切れ・白紙の恐怖ゼロ）
② 人間：✍️印の「体験」「ひとひねり」を5分で足す  ← ここが“買う価値”＝売れる分かれ目
③ 人間：note に手で投稿（規約セーフ）          ← 最初の反応を見て改善できる
④ 人間：売れたテーマを次に寄せる               ← 勝ちパターンを育てる
```

`②` の記入欄は、下書きの中に **✍️印** で最初から埋め込まれています。
`npm run paste` で「チェックリスト＋note貼り付け用テキスト」がそのまま出ます。
...
# ★おすすめ★ note貼り付け用テキスト＋投稿前チェックリストを出す
npm run paste            # 今日(JST)
npm run paste 2026-12-25 # 指定日
...
## note.com への自動投稿（⚠️ 非推奨・実験用）

> ⚠️ **原則おすすめしません。** note.com に公式の投稿APIはなく、これは Playwright による
> ブラウザ自動操作です。利用規約上グレー〜NGで、毎日の自動投稿は**アカウント凍結**の恐れがあります。
> しかも、テンプレ記事をそのまま自動投稿しても**まず売れません**。
> **投稿は `npm run paste` の出力を手でコピペするのが、規約セーフで売上も伸びる正攻法です。**
> 以下は「仕組みとして可能か試したい人」向けの実験機能で、既定では安全弁により何も投稿しません。
